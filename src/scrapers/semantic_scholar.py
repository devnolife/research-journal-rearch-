...
Dengan retry logic dan exponential backoff untuk menangani rate limiting
"""
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[DEBUG] Rate limited (429). Retry {attempt + 1}/{MAX_RETRIES} after {delay:.1f}s..."
                    )
                    time.sleep(delay)
                    ...
                    print(
                        f"[DEBUG] Server error ({response.status_code}). Retry {attempt + 1}/{MAX_RETRIES} after {delay:.1f}s..."
                    )
                    time.sleep(delay)

                else:
                    print(
                        f"[DEBUG] Semantic Scholar API returned status {response.status_code}"
                    )
                    break

            except requests.exceptions.Timeout:
                print(f"[DEBUG] Request timeout. Retry {attempt + 1}/{MAX_RETRIES}...")
                time.sleep(BASE_DELAY)
            except requests.exceptions.RequestException as e:
                print(
                    f"[DEBUG] Request error: {e}. Retry {attempt + 1}/{MAX_RETRIES}..."
                )
                time.sleep(BASE_DELAY)
