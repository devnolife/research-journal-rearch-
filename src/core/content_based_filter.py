...
class ContentBasedFilter:
    ...
    def get_similar_papers(self, paper_index, top_n=5):
        ...
        scored_papers = [
            (i, score) for i, score in enumerate(similarities) if i != paper_index
        ]
        scored_papers.sort(key=lambda x: x[1], reverse=True)
    ...
    def get_tfidf_terms(self, paper_index, top_n=10):
        ...
        top_terms = [
            (feature_names[i], round(paper_vector[i], 4))
...
def get_cbf_calculation_details(selected_papers, query, max_papers=10):
    """
    ...
    Args:
        selected_papers: Papers yang dipilih user
        query: Search query
        max_papers: Maximum papers to process (default 10 for performance)
    ...
    papers_to_process = selected_papers[:max_papers]

    cbf = ContentBasedFilter()
    cbf.fit(papers_to_process)
    ...
    tfidf_data = {
        "total_terms": 0,
        "total_docs": len(papers_to_process),
        "top_terms": [],
        "matrix": [],
        "matrix_full": [],  # Full TF-IDF matrix for visualization
        "feature_names": [],  # All term names
    }
    ...
    if cbf.vectorizer and cbf.tfidf_matrix is not None:
        ...
        tfidf_data["feature_names"] = list(
            feature_names[:50]
        )  # Top 50 terms for display
        ...
        # Generate full TF-IDF matrix (documents x top terms) for table display
        tfidf_matrix_array = cbf.tfidf_matrix.toarray()
        top_term_indices = top_indices[:20]  # Top 20 terms for matrix

        for i, paper in enumerate(papers_to_process[:10]):  # Max 10 documents
            doc_row = {
                "doc_id": f"D{i + 1}",
            ...
            for idx in top_term_indices:
                term = feature_names[idx]
                value = float(tfidf_matrix_array[i][idx])
                if value > 0:  # Only include non-zero values
                    doc_row["values"][term] = round(value, 4)
            tfidf_data["matrix_full"].append(doc_row)

        # Terms list for matrix header
        tfidf_data["matrix_terms"] = [feature_names[idx] for idx in top_term_indices]
    ...
    ranking = sorted(
        papers_with_similarity, key=lambda x: x["similarity"], reverse=True
    )

    # Similarity matrix between papers
    similarity_matrix = []
    if cbf.tfidf_matrix is not None and len(papers_to_process) > 1:
        sim_matrix = cosine_similarity(cbf.tfidf_matrix)
        for i in range(min(len(papers_to_process), 10)):
            row = []
            for j in range(min(len(papers_to_process), 10)):
    ...
    cosine_details = []
    if cbf.tfidf_matrix is not None and len(papers_to_process) > 0:
        query_preprocessed = cbf.preprocess_text(query)
        try:
            query_vector = (
                cbf.vectorizer.transform([query_preprocessed]).toarray().flatten()
            )
            ...
            for i in range(min(len(papers_to_process), 10)):
                ...
                cosine_details.append(
                    {
                        "doc_id": f"D{i + 1}",
                        "title": papers_to_process[i].get("title", "")[:60],
    ...
    calculation_details = {
        ...
        "tfidf_info": {
            ...
            "example": f'Contoh: Jika term "learning" muncul di 3 dari {len(papers_to_process)} dokumen, IDF = log({len(papers_to_process)}/3) + 1',
        },
        "cosine_info": {
    ...
    for i, paper in enumerate(papers_to_process):
        ...
        if i < len(cbf.paper_texts):
            ...
            paper_analysis["preprocessed_sample"] = (
    ...
    scores = [p.get("relevance_score", 0) for p in papers_to_process]
    calculation_details["statistics"] = {
        ...
        "total_unique_terms": len(cbf.vectorizer.get_feature_names_out())
        if cbf.vectorizer
        else 0,
    }
...
if __name__ == "__main__":
    ...
    results = cbf.calculate_similarity_to_query(
